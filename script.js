...
const problems = [
  { title: 'Belt runs off at tail pulley', codes: [7, 15, 14, 17, 21, 34] },
  { title: 'Entire belt runs off at all points of the line', codes: [26, 17, 15, 21, 4, 16] },
  { title: 'One belt section runs off at all points on the line', codes: [2, 11, 1, 34] },
  { title: 'Belt runs off at head pulley', codes: [15, 22, 21, 16, 34] },
  { title: 'Belt runs to one side throughout entire length at specific idlers', codes: [15, 16, 21, 34] },
  { title: 'Belt slip', codes: [19, 7, 21, 14, 22] },
  { title: 'Belt slip on starting', codes: [19, 7, 22, 10] },
  { title: 'Excessive belt stretch', codes: [13, 10, 21, 6, 9] },
  { title: 'Belt breaks at or behind fasteners or fasteners tear loose', codes: [2, 23, 13, 22, 20, 10] },
  { title: 'Vulcanized splice separation', codes: [13, 23, 10, 20, 2, 9] },
  { title: 'Excessive belt wear including rips, gouges, ruptures and tears', codes: [12, 25, 17, 21, 8, 5] },
  { title: 'Excessive belt bottom cover wear', codes: [21, 14, 5, 19, 20, 22] },
  { title: 'Excessive belt edge wear, broken edges', codes: [26, 4, 17, 8, 1, 21] },
  { title: 'Belt cover swells in spots or streaks', codes: [8] },
  { title: 'Belt hardens or cracks', codes: [8, 23, 22, 18] },
  { title: 'Belt covers become checked or brittle', codes: [8, 18] },
  { title: 'Longitudinal grooving or cracking of belt top cover', codes: [27, 14, 21, 12] },
  { title: 'Longitudinal grooving or cracking of belt bottom cover', codes: [14, 21, 22] },
  { title: 'Belt fabric decay, carcass cracks, ruptures, gouges (soft spots in belt)', codes: [12, 20, 5, 10, 8, 24] },
  { title: 'Belt ply separation', codes: [13, 23, 11, 8, 3] },
  { title: 'Build up on bend pulleys and return idlers', codes: [32, 33, 8, 22] },
  { title: 'Spillage of fines and small particles in loading area', codes: [27, 28, 17, 12, 30] },
  { title: 'Spillage of larger particles and lumps along conveyor', codes: [15, 29, 30, 31, 35] },
  { title: 'Plugged chutes', codes: [35, 33, 34, 31] },
  { title: 'Damage to accessories in contact with the belt', codes: [31, 2, 11] },
];
...
const menuCards = document.querySelectorAll('.menu-card.focusable');
  ...
  codes.forEach((code, index) => {
    ...
    cloud.innerHTML = `
      <div class="cause-code">
        <span class="cause-rank">#${index + 1} most likely</span>
        <span class="cause-title">${cause.title}</span>
        <span class="cause-meta">Cause code ${code} · tap to see fixes</span>
      </div>
    `;
...
function selectCause(code, cloud) {
  ...
  meta.className = 'cause-meta';
  meta.textContent = `Reference code ${cloud.dataset.code}`;
  const body = document.createElement('p');
  body.className = 'solution-body';
  body.textContent = cause.solution;

  solutionPanel.appendChild(title);
  solutionPanel.appendChild(meta);
...
function bindMenuNavigation() {
  ...
  [...menuCards, ...ctaButtons].forEach((el) => {
